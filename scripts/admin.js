...
document.getElementById('postNew').addEventListener('click', async (e) => {
  ...
  const author = document.getElementById('postAuthor').value || (auth.currentUser ? auth.currentUser.email.split('@')[0] : 'Anonymous');
...
document.getElementById('downloadMd').addEventListener('click', () => {
  ...
  const postAuthor = document.getElementById('postAuthor').value || userEmail.split('@')[0];
  ...
  a.href = url;
  a.download = `${postTitle.replace(/ /g, '_')}-${Math.random().toString(36).slice(2, 7)}.md`;
    ...
    mdFileInput.addEventListener('change', function (event) {
      ...
      if (file) {
        ...
        reader.onload = function (e) {
          ...
          if (titleMatch) document.getElementById('postTitle').value = titleMatch[1].trim();
          if (authorMatch) document.getElementById('postAuthor').value = authorMatch[1].trim();
          if (hideMatch) document.getElementById('postHide').checked = hideMatch[1].trim().toLowerCase() === 'true';
